...
void parse()
{
    ...
    {
        if(isdtype() || (token==STRUCT)) // 是定义的类型 或者 是一个结构体
        {
            if(token==INT && token_table[ptokenno].tokenval == MAIN)    //如果是int main
            {
                root_tag++;
            ...
            {
                root_tag++;
                newtemp=newlnodestmt(funck);/// 函数  动态申请一条语句结构的节点
                match(token);
            ...
            {//struct Student(
                root_tag++;
                newtemp=newlnodestmt(structk);  //  结构体
                match(token);
                treenode *j=newlnodeexp(idk);
                match(ID);
                newtemp->child[0]=j;
            ...
            {
                root_tag++;
                newtemp=newlnodestmt(defineparak);//para 变量
